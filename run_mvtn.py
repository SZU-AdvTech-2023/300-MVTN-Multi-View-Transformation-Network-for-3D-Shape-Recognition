...
multiscale_sammple = [1024, 2048, 4096]

parser = argparse.ArgumentParser(description='MVTN-PyTorch')

parser.add_argument('--data_dir', default='/mnt/d/data/ModelNet40', help='path to 3D dataset')
...
parser.add_argument('--nb_views', type=int, default=6,
                    help='number of views in the multi-view setup')
parser.add_argument('--views_config', '-s', default="learned_spherical",
                    choices=["circular", "random", "learned_circular", "learned_direct", "spherical",
                             "learned_spherical", "learned_random", "learned_transfer", "custom"],
...
parser.add_argument('--object_color', '-clr', default="white",
...
if "modelnet" in setup["data_dir"].lower():
    dset_train = ModelNet40(setup["data_dir"], "train", nb_points=setup["nb_points"],
                            # simplified_mesh=setup["simplified_mesh"], cleaned_mesh=setup["cleaned_mesh"],
                            simplified_mesh=setup["simplified_mesh"],
                            dset_norm=setup["dset_norm"], return_points_saved=setup["return_points_saved"],
                            is_rotated=setup["rotated_train"])
    dset_val = ModelNet40(setup["data_dir"], "test", nb_points=setup["nb_points"],
                          # simplified_mesh=setup["simplified_mesh"], cleaned_mesh=setup["cleaned_mesh"],
                          simplified_mesh=setup["simplified_mesh"],
                          dset_norm=setup["dset_norm"], return_points_saved=setup["return_points_saved"],
...
def train(data_loader, models_bag, setup):
    ...
    for i, (targets, meshes, points_n) in enumerate(data_loader):
        c_batch_size = targets.shape[0]
        models_bag["optimizer"].zero_grad()
        if setup["is_learning_views"]:
            models_bag["mvtn_optimizer"].zero_grad()

        # [B, 3] -> [B, 9] for sample 2048 points to sample 1024, 2048 and 4096 respectively!

        loss = 0
        for points in points_n:
...
def evluate(data_loader, models_bag, setup, is_test=False, retrieval=False):
    ...
    t = tqdm(enumerate(data_loader), total=len(data_loader))
    for i, (targets, meshes, points_n) in t:
...
def view_gcn_exp(setup, models_bag, train_loader, val_loader, dset_val):
    ...
    trainer = ModelNetTrainer_mvt(models_bag, train_loader, val_loader, dset_val,
                                  nn.CrossEntropyLoss(), 'view-gcn', setup["checkpoint_dir2"],
                                  num_views=setup["nb_views"], setup=setup, classes=classes)
        ...
        for epoch in range(setup["start_epoch"], n_epochs):
            ...
            if (epoch + 1) % setup["plot_freq"] == 0:
                for indx, ii in enumerate(PLOT_SAMPLE_NBS):
                    c_batch_size = 1
                    (targets, meshes, points_n) = dset_val[ii]
                    ...
                    for sample_point_idx, points in enumerate(points_n):
                        cameras_path = os.path.join(
                            cameras_root_folder,
                            "MV_cameras_{}_{}.jpg".format(str(multiscale_sammple[sample_point_idx]), str(epoch + 1)))
                        images_path = os.path.join(
                            renderings_root_folder,
                            "MV_renderings_{}_{}.jpg".format(str(multiscale_sammple[sample_point_idx]), str(epoch + 1)))
