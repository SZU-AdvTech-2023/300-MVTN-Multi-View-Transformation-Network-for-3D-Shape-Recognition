...
# from .point_transformer_cls import PointTransformerCls, Bottleneck
from .point_tnt import Baseline
...
class FeatureExtractor(nn.Module):
    def __init__(self,  shape_features_size, views_config, shape_extractor, screatch_feature_extractor=False):
        ...
        #     self.features_origin = "pre_extracted"
            ...
            if shape_extractor == "PointNet":
                ...
                self.fe_model = Baseline(40)
                # self.fe_model = SimpleDGCNN(40)
            elif shape_extractor == "DGCNN":
                self.fe_model = SimpleDGCNN(40)
            # elif shape_extractor == "attention":
            #     blocks = [1] * 5
                # self.shape_extractor, = PointTransformerCls(Bottleneck, blocks)
    ...
    def forward(self, extra_info=None, c_batch_size=1):
        ...
        #     return extra_info.view(c_batch_size, self.shape_features_size)
            ...
            return features.view(c_batch_size, -1)
...
class MVTN(nn.Module):
    ...
    def forward(self, points=None, c_batch_size=1):
        # here using Point Encoder to extract point feature. Hence, I need to replace it with self-attention model!
        shape_features = self.feature_extractor(points, c_batch_size) # [batch_size, 40], points to 40
