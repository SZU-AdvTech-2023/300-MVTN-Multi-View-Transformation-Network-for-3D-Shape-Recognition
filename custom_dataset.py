    ...
    def __getitem__(self, index):
        ...
        if self.return_points_sampled or self.return_points_saved:
            if self.return_points_sampled:
                # here need to sample point with 4096, 2048, 1024 respectively.
                points0 = threeobject.sample(1024, False)
                points = threeobject.sample(self.nb_points, False)
                points1 = threeobject.sample(4096, False)
            else:
            ...
            points0 = torch.from_numpy(rotation_matrix(
                rot_axis, angle).dot(points0.T).T).to(torch.float)
            points0 = torch_center_and_normalize(points0, p=self.dset_norm)
            points1 = torch.from_numpy(rotation_matrix(
                rot_axis, angle).dot(points1.T).T).to(torch.float)
            points1 = torch_center_and_normalize(points1, p=self.dset_norm)
        # return self.y[index], mesh, points
        return self.y[index], mesh, [points0, points, points1]
